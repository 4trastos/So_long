			...
			game->map[game->posy / SIZE + UP][game->posx / SIZE] == 'E')
	{
		if ((game->posx / SIZE) % 2 == 0)
			mlx_put_image_to_window(game->mlx, game->new_w,
				game->grass, game->posx, game->posy);
		else
			mlx_put_image_to_window(game->mlx, game->new_w,
				game->grasstwo, game->posx, game->posy);
		...
		ft_moveup(game);
		mlx_put_image_to_window(game->mlx, game->new_w,
			game->playerup, game->posx, game->posy);
	}
}
...
void	move_s(t_game *game)
{
			...
			game->map[game->posy / SIZE + DOWN][game->posx / SIZE] == 'C')
	{
		...
		ft_movedown(game);
		mlx_put_image_to_window(game->mlx, game->new_w,
			game->playerdown, game->posx, game->posy);
	}
}
...
void	move_a(t_game *game)
{
			...
			game->map[game->posy / SIZE][game->posx / SIZE - 1] == 'C')
	{
		...
		ft_moveleft(game);
		mlx_put_image_to_window(game->mlx, game->new_w,
			game->playerleft, game->posx, game->posy);
	}
}
...
void	move_d(t_game *game)
{
			...
			game->map[game->posy / SIZE][game->posx / SIZE + 1] == 'C')
	{
		...
		ft_moveright(game);
		mlx_put_image_to_window(game->mlx, game->new_w,
			game->playeright, game->posx, game->posy);
	}
}
